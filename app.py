...
# सिम्बॉल्स यादी (Indices + Top F&O Stocks)
watch_list = {
    "NIFTY 50": "^NSEI",
    "SENSEX": "^BSESN",
    "BANK NIFTY": "^NSEBANK",
...
# डेटा फेचिंग आणि EMA कॅल्क्युलेशन
def check_ema_touch(ticker, interval):
    # 1m साठी कमाल 7 दिवस, 5m/60m साठी 30 दिवस उपलब्ध असतात
    ...
    # मल्टि-इंडेक्स कॉलम्स सपाट करणे (yfinance अपडेटनुसार)
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)

    # EMA कॅल्क्युलेशन (9, 21, 50, 200)
    ...
    for ema_val, name in [(latest["EMA_9"], "EMA 9"), 
        ...
        if pd.notna(ema_val):
            # कॅण्डलचा Low आणि High या दरम्यान EMA आहे का (म्हणजे टच झाला आहे का)
...
if st.sidebar.button("स्कॅनर सुरू करा (Scan Now)"):
    ...
    for i, (name, symbol) in enumerate(watch_list.items()):
        ...
        if touched:
            ...
            results.append({
                "नाव / सिम्बॉल": name,
                "सध्याची किंमत (LTP)": round(last_close, 2),
...
if chart_symbol_name:
    ...
    if chart_df is not None and not chart_df.empty:
        plot_df = chart_df.tail(100) # शेवटच्या १०० कॅण्डल्स
